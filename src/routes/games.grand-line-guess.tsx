              ...
              <>
                ...
                {!state?.solved && state?.status === "active" && (
                  <div className="mt-3 flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">Hints reduce reward by 25% each.</span>
                    <button
                      onClick={() => hintM.mutate()}
                      disabled={hintM.isPending || (state?.hints_used ?? 0) >= 3}
                      className="border border-border px-3 py-2 uppercase tracking-widest text-foreground hover:border-primary disabled:opacity-40"
                    >
                      Use hint
                    </button>
                  </div>

  ...
  head: () => ({
    meta: [
      { title: "Leaderboards — Berry Street" },
      {
        name: "description",
        content:
          "Top investors on the Grand Line stock market — net worth, returns, accuracy, and per-character holders.",
      },
    ],
  }),
...
const BOARDS = [
  {
    key: "net_worth_all_time",
    label: "Net Worth",
    suffix: "berries",
    value: (v: number) => `฿${formatBerries(v)}`,
  },
  {
    key: "return_all_time",
  ...
  },
  {
    ...
    label: "30-Day Return",
    suffix: "%",
    value: (v: number) => `${v >= 0 ? "+" : ""}${Number(v).toFixed(2)}%`,
  },
  {
    key: "return_7d",
    label: "7-Day Return",
  ...
  },
  {
    ...
    label: "Most Accurate",
    suffix: "%",
    value: (v: number) => `${v.toFixed(1)}% win rate`,
  },
  {
    key: "most_active",
    label: "Most Active",
    suffix: "trades",
    value: (v: number) => `${v} trades`,
  },
] as const;
    ...
    <TerminalShell>
      <div className="border-b border-border bg-card/60 px-4 py-3">
        ...
        <p className="text-[11px] text-muted-foreground">
          Rankings refresh daily at 00:15 UTC. Status climbs with reputation.
        </p>
      </div>
          ...
          <div className="terminal-panel overflow-hidden">
            ...
            {rows.isLoading ? (
              ...
              <div className="p-6 text-xs text-muted-foreground">
                No qualifying investors yet on this board.
              </div>
            ) : (
              <table className="w-full text-xs tabular">
                ...
                <tbody>
                  {(rows.data ?? []).map((r) => {
                    ...
                    const privateValue = r.is_public === false || r.value == null;
                    return (
                      <tr
                        key={r.username}
                      ...
                      >
                        ...
                        <td className="px-3 py-2">
                          ...
                          {r.display_name && (
                            <span className="ml-2 text-muted-foreground">{r.display_name}</span>
                          )}
                        </td>
                        <td
                          className={`px-3 py-2 text-[10px] uppercase tracking-widest ${TITLE_TONE[r.title] ?? ""}`}
                        >
                          {TITLE_LABEL[r.title] ?? r.title}
                        </td>
                        <td className="px-3 py-2 text-right">
                          {privateValue ? (
                            <span className="inline-flex items-center border border-border px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
                              Private
                            </span>
                          ) : (
                            board.value(Number(r.value))
                          )}
                        </td>
              ...
              <ul className="divide-y divide-border text-xs">
                {movement.data!.climbers.map((c) => (
                  <li key={c.username} className="flex items-center justify-between px-3 py-2">
                    ...
                    <span className="text-bull tabular">
                      ▲ {c.delta} → #{c.rank}
                    </span>
                  </li>
                ))}
              ...
              <ul className="divide-y divide-border text-xs">
                {movement.data!.fallers.map((c) => (
                  <li key={c.username} className="flex items-center justify-between px-3 py-2">
                    ...
                    <span className="text-bear tabular">
                      ▼ {Math.abs(c.delta)} → #{c.rank}
                    </span>
                  </li>
                ))}
              </ul>

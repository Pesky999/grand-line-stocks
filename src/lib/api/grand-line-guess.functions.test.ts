...
const source = readFileSync(
  join(process.cwd(), "src/lib/api/grand-line-guess.functions.ts"),
  "utf8",
);
const routeSource = readFileSync(
  join(process.cwd(), "src/routes/games.grand-line-guess.tsx"),
  "utf8",
);
...
test("wrong guesses do not deduct wallet funds or call the reward path", () => {
  ...
  assert.doesNotMatch(
    wrongGuessBranch,
    /awardGrandLineGuessReward|awardGrandLineGuessRewardSafely|ensureGrandLineGuessRewardWallet|refreshGrandLineGuessProgressionSafely|refresh_user_progression|user_wallets|berries\s*[-=]/,
  );
});
...
test("correct guesses use the award path and payout failures return contained state", () => {
  ...
  assert.match(
    correctGuessBranch,
    /if \(rewardError\) \{[\s\S]*return applyPayoutFailure\(state, rewardError\);[\s\S]*\}\s+await refreshGrandLineGuessProgressionSafely\(db, userId\);/,
  );
  assert.match(source, /async function awardGrandLineGuessRewardSafely/);
  assert.match(
    source,
    /type PayoutFailure = \{[\s\S]*payout_error_code: string[\s\S]*payout_error_step: PayoutErrorStep/,
  );
...
test("successful Grand Line Guess solves refresh progression as a best-effort server side effect", () => {
  const refreshHelper = between(
    source,
    "async function refreshGrandLineGuessProgressionSafely",
  ...
  assert.doesNotMatch(refreshHelper, /throw new GrandLineGuessPayoutError|throw error/);
});

test("loadState exposes persistent unpaid solved payout state", () => {
  const loadStateBlock = between(
    source,
    "async function loadState",
    "export const getTodayGrandLineGuessState",
  );

  assert.match(
    loadStateBlock,
    /const correctAttempt =\s+attempts\.find\(\(attempt: \{ is_correct: boolean \}\) => attempt\.is_correct\) \?\? null/,
  );
  assert.match(
    loadStateBlock,
  ...
  assert.match(
    loadStateBlock,
    /const rewardPayoutPending = Boolean\(\s+\(result\?\.solved && !rewardPaid\) \|\| \(correctAttempt && !rewardPaid\),\s+\)/,
  );
  assert.match(
    loadStateBlock,
    /const pendingRewardAmount =\s+rewardPayoutPending && correctAttemptNumber != null[\s\S]*rewardForAttempt\(correctAttemptNumber\)/,
  );
...
test("reward payout prepares a missing wallet row without touching balances", () => {
  ...
  assert.match(
    helper,
    /\.from\("user_wallets"\)[\s\S]*\.select\("user_id"\)[\s\S]*\.eq\("user_id", userId\)[\s\S]*\.maybeSingle\(\)/,
  );
  ...
  assert.match(
    helper,
    /const raced = await db[\s\S]*\.from\("user_wallets"\)[\s\S]*\.select\("user_id"\)/,
  );
  assert.doesNotMatch(helper, /\.update\(|\.upsert\(|berries/);
  assert.match(
    source,
    /await ensureGrandLineGuessRewardWallet\(db, args\.userId\);\s+await awardGrandLineGuessReward\(db, args\);/,
  );
});
...
test("wallet precondition failures map to safe diagnostic codes", () => {
  ...
  assert.match(
    helper,
    /GrandLineGuessPayoutError\("WALLET_PRECHECK_SELECT_FAILED", existing\.error\.code\)/,
  );
  assert.match(
    helper,
    /GrandLineGuessPayoutError\("WALLET_PRECHECK_RACE_RECHECK_FAILED", raced\.error\.code\)/,
  );
  assert.match(
    helper,
    /GrandLineGuessPayoutError\("WALLET_PRECHECK_INSERT_FAILED", created\.error\.code\)/,
  );
});

test("reward RPC keeps compatibility arguments and logs safe diagnostics on failure", () => {
  const awardFn = between(
    source,
    "async function awardGrandLineGuessReward(",
...
test("server RPC argument names match the restored migration signature", () => {
  ...
  for (const argumentName of ["_puzzle_id", "_user_id", "_attempt_number", "_reward_amount"]) {
    assert.match(
      awardFn,
      new RegExp(`${argumentName}:`),
      `${argumentName} is passed by the server`,
    );
    assert.match(
      migration,
      new RegExp(`${argumentName} (?:uuid|integer)`),
      `${argumentName} exists in the RPC signature`,
    );
  }
});

test("retry payout uses existing correct attempt and never records a new guess", () => {
  const retryFn = between(
    source,
    "export const retryGrandLineGuessReward",
    "export const submitGrandLineGuess",
  );

  assert.match(retryFn, /\.middleware\(\[requireSupabaseAuth\]\)/);
  assert.match(
    retryFn,
    /\.from\("grand_line_guess_attempts"\)[\s\S]*\.eq\("is_correct", true\)[\s\S]*\.order\("attempt_number", \{ ascending: true \}\)[\s\S]*\.limit\(1\)/,
  );
  assert.match(
    retryFn,
    /if \(!correctAttempt\) \{[\s\S]*reward_error: "No unpaid Grand Line Guess reward is available to retry\."/,
  );
  ...
  assert.match(
    retryFn,
    /awardGrandLineGuessRewardSafely\(db, \{[\s\S]*attemptNumber: correctAttempt\.attempt_number[\s\S]*rewardAmount: rewardForAttempt\(correctAttempt\.attempt_number\)/,
  );
...
test("UI shows a generic payout failure diagnostic without manual retry controls", () => {
  ...
  assert.doesNotMatch(
    routeSource,
    /Your correct answer was recorded, but the reward has not been paid yet\./,
  );
...
test("duplicate correct submissions retry the idempotent award path without trusting the client", () => {
  ...
  assert.match(
    duplicateBranch,
    /if \(!rewardError\) \{[\s\S]*await refreshGrandLineGuessProgressionSafely\(db, userId\);[\s\S]*\}/,
  );
...
test("new migration replaces the authoritative RPC formula and accepts reward 0", () => {
  ...
  assert.match(
    migration,
    /CREATE OR REPLACE FUNCTION public\.award_grand_line_guess_reward\(\s*_puzzle_id uuid,\s*_user_id uuid,\s*_attempt_number integer,\s*_reward_amount integer\s*\)/,
  );
  ...
  assert.match(
    migration,
    /v_wrong_guesses := CASE\s+WHEN v_attempt_number > 1 THEN v_attempt_number - 1\s+ELSE 0\s+END/,
  );
  assert.match(
    migration,
    /v_computed_reward := CASE\s+WHEN 1000 - \(100 \* v_wrong_guesses\) > 0 THEN 1000 - \(100 \* v_wrong_guesses\)\s+ELSE 0\s+END/,
  );
  assert.doesNotMatch(
    migration,
    /WHEN v_attempt_number = 1 THEN 750|WHEN v_attempt_number = 2 THEN 600|WHEN v_attempt_number = 3 THEN 500/,
  );
...
test("new migration ignores client reward amounts and pays the database-calculated reward", () => {
  ...
  assert.match(
    migration,
    /INSERT INTO public\.user_wallets \(user_id\)[\s\S]*ON CONFLICT \(user_id\) DO NOTHING/,
  );
  assert.match(
    migration,
    /FROM public\.user_wallets[\s\S]*WHERE user_id = _user_id[\s\S]*FOR UPDATE/,
  );
...
test("new migration keeps duplicate-payout protection and service-role-only execution", () => {
  ...
  assert.match(
    migration,
    /UPDATE public\.user_wallets\s+SET berries = berries \+ v_computed_reward/,
  );
  assert.match(
    migration,
...
test("new migration avoids unsafe reward-history writes and invalid special expressions", () => {
  ...
  assert.doesNotMatch(
    ...
    /\buser_holdings\b|\bprice_history\b|\bdaily_crew\b/i,
  );

...
COMMENT ON COLUMN public.characters.is_listed IS
  'False while an administrator prepares a private draft; true only after an atomic IPO publication.';
...
BEGIN
  ...
  END IF;
  IF v_version IS NULL OR v_version <> '1.2.0' THEN
    RAISE EXCEPTION 'Unsupported pricing algorithm version' USING ERRCODE = '22023';
  END IF;

  /*
    Market Pricing V1.2.0 parity with src/lib/market-pricing/v1.ts.
    Volatility is persisted but excluded from fundamental fair value.
    The approved 1.50 rubric stretches the weighted score around the fixed 79.85 anchor.
  */
  v_weighted_score :=
  ...
  RETURN QUERY SELECT v_weighted_score, v_pricing_score, v_applied_price;
END;
$function$;

REVOKE ALL ON FUNCTION public.calculate_market_price_v1_2(
...
BEGIN
  ...
  PERFORM * FROM public.calculate_market_price_v1_2(
    _narrative_importance, _current_relevance, _strength_status, _popularity,
    _future_potential, _investor_confidence, _volatility, _comparable_adjustment,
    _uncertainty_discount_pct, _launch_catalyst_pct, v_version
  );
  ...
  VALUES (
    _character_id, _narrative_importance, _current_relevance, _strength_status,
    _popularity, _future_potential, _investor_confidence, _volatility, _stock_category,
    _comparable_adjustment, _uncertainty_discount_pct, _launch_catalyst_pct,
    v_version, 'draft', v_user, v_user, NULL, NULL
  )
...
BEGIN
  ...
  IF v_character.id IS NULL THEN
    RAISE EXCEPTION 'Character not found';
  END IF;
  IF NOT v_character.is_listed THEN
    RAISE EXCEPTION 'Character is not listed; publish its IPO instead';
  END IF;

  SELECT calculated.applied_price INTO v_applied_price
  FROM public.calculate_market_price_v1_2(
    _narrative_importance, _current_relevance, _strength_status, _popularity,
    _future_potential, _investor_confidence, _volatility, _comparable_adjustment,
    _uncertainty_discount_pct, _launch_catalyst_pct, v_version
  ) AS calculated;
  ...
  VALUES (
    ...
    _popularity, _future_potential, _investor_confidence, _volatility, _stock_category,
    _comparable_adjustment, _uncertainty_discount_pct, _launch_catalyst_pct,
    v_version, 'approved', v_user, v_user, v_now, v_user
  )
  ON CONFLICT (character_id) DO UPDATE SET
    narrative_importance = EXCLUDED.narrative_importance,
    current_relevance = EXCLUDED.current_relevance,
  ...
  VALUES (
    _character_id, v_applied_price,
    'Market Pricing applied valuation using algorithm ' || v_version,
    v_percentage_change, 'pricing_rebase'
  )
...
BEGIN
  ...
  IF v_character.id IS NULL THEN
    RAISE EXCEPTION 'Character not found';
  END IF;
  IF v_character.is_listed THEN
    RAISE EXCEPTION 'Character is already listed; use repricing instead';
  END IF;
  ...
  UPDATE public.characters AS c
  SET previous_price = v_applied_price,
      current_price = v_applied_price,
      category = _stock_category,
      momentum = 0,
      is_listed = true
  ...
  VALUES (
    _character_id, v_applied_price,
    'Initial public offering using Market Pricing algorithm ' || v_version,
    0, 'ipo'
  )
  RETURNING id INTO v_price_history_id;
  ...
  RETURN jsonb_build_object(
    ...
    'priceHistoryId', v_price_history_id,
    'pricingAlgorithmVersion', v_version,
    'previousLivePrice', v_applied_price,
    'newLivePrice', v_applied_price,
    'percentageChange', 0,
    'previousCategory', v_character.category,
    'newCategory', _stock_category
...
DROP TRIGGER IF EXISTS transactions_require_listed_character ON public.transactions;
CREATE TRIGGER transactions_require_listed_character
  BEFORE INSERT ON public.transactions
  FOR EACH ROW EXECUTE FUNCTION public.reject_unlisted_character_transaction();

REVOKE ALL ON FUNCTION public.reject_unlisted_character_transaction()
  FROM PUBLIC, anon, authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.publish_character_ipo(
  uuid, integer, integer, integer, integer, integer, integer, integer,
  public.stock_category, numeric, numeric, numeric, text
) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.publish_character_ipo(
  uuid, integer, integer, integer, integer, integer, integer, integer,
  public.stock_category, numeric, numeric, numeric, text
) TO authenticated, service_role;

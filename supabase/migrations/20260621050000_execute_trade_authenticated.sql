-- Authenticated trading entry point.
-- Derives the acting user from auth.uid() and delegates to the existing
-- service-role-only atomic trading core in the same database transaction.
...
BEGIN
  ...
  -- Hold a share lock so price validation cannot race a concurrent price update
  -- before the internal trade function snapshots the same character row.
...
-- Reassert that the internal user-id-taking trade core is not directly callable
-- by browser/authenticated roles.
